    ...
    const chat = async (prompt: string) => {
        ...
        try {
            ...
            if (response.status === 500) {
                ...
                setMessages((prev) => [...prev, { role: 'assistant', content: "Cant connect to Ollama. Please try again later." }]);
                return;
            }
            ...
            setMessages((prev) => [...prev, { role: 'assistant', content: "An error occurred while fetching data. Please try again." }]);
        }
    }
        ...
        <div className="w-full h-[calc(100vh-93px)]">
            ...
            <div className="flex justify-center items-center h-full w-full mt-auto">
                ...
                <ul className="text-sm gap-2 flex flex-col items-start justify-start w-full text-pretty max-w-2xl text-neutral-300 ">
                    <li onClick={() => chat("Open FaceFusion")} className="bg-white/10 px-4 py-1 rounded-lg flex gap-2 items-center hover:text-neutral-100 cursor-pointer transition-colors duration-200">"Open FaceFusion" <ArrowRight className="ml-2" size={16} /></li>
                    <li onClick={() => chat("Install Applio")} className="bg-white/10 px-4 py-1 rounded-lg flex gap-2 items-center hover:text-neutral-100 cursor-pointer transition-colors duration-200">"Install Applio" <ArrowRight className="ml-2" size={16} /></li>
                    <li onClick={() => chat("What is the latest application in Dione?")} className="bg-white/10 px-4 py-1 rounded-lg flex gap-2 items-center hover:text-neutral-100 cursor-pointer transition-colors duration-200">"What is the latest application in Dione?" <ArrowRight className="ml-2" size={16} /></li>
                </ul>
                ): (
            ...
            <div className="w-full max-w-2xl h-full flex flex-col justify-end mx-auto items-center">
                ...
                />
                    ...
                    <div className="sticky max-w-2xl h-15 bg-white/5 backdrop-blur-3xl border border-white/5 w-full rounded-xl overflow-hidden">
                        <input className="w-full h-full outline-none border-none bg-transparent text-white px-4" type="text" placeholder="Ask Dio..." 
                        autoFocus
                        onKeyDown={(e) => {
                            if (e.key === "Enter") {
