	...
	const handleReportError = async () => {
		...
		if (!error) {
			...
			const genericError = new Error(
				"User-initiated report without a caught component error.",
			);
			reportPayload = {
				...
				userDescription:
					userDescription || "No additional user description provided.",
			};
		...
		try {
			...
			alert(
				"Failed to send report. Please check your connection or try again later.",
			);
		}
	};
		...
		<div className="min-h-screen pt-4 overflow-hidden bg-neutral-900 text-neutral-100 flex flex-col items-center justify-center">
			<div className="max-w-2xl w-full mx-auto px-4 sm:px-6 lg:px-8 text-center">
				<main className="flex flex-col items-center pb-12">
					<Icon
						name="DioDead"
						className="h-36 w-36 sm:h-44 sm:w-44 mb-8 sm:mb-12 text-red-500"
					/>
					...
					<p className="text-neutral-400 text-sm sm:text-base max-w-md text-center text-pretty mb-6">
						We've encountered an unexpected issue. You can try returning home,
						or help us by describing what happened and sending a report.
					</p>

					{error && (
						<div className="mb-6 p-3 bg-neutral-800 border border-neutral-700 rounded-md text-left w-full max-w-md">
							<h3 className="text-md font-semibold text-red-400 mb-1">
								Error Details:
							</h3>
					...
					<button
						onClick={() =>
							openLink(
								"https://github.com/dioneapp/dioneapp/issues/new?template=BUG.yml",
							)
						}
						type="button"
