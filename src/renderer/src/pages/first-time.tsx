        ...
        <div className="absolute w-screen h-screen inset-0 z-50 bg-[#080808]/5 overflow-hidden">
            ...
            {level === 1 && (
                <div className="w-full h-full flex flex-col  items-center justify-center">
                    ...
                    <button className="bg-white/10 w-28 rounded-full p-1.5 text-sm text-neutral-300 hover:bg-white/20 transition-colors duration-300 cursor-pointer" onClick={() => { setLevel(2); openLink("https://getdione.app/auth/login"); }}>
                        Log in
                    </button>
            ...
            {level === 2 && (
                <div className="w-full h-full flex flex-col  items-center justify-center">
                    ...
                    <h2 className="text-neutral-400 text-balance text-center max-w-xl">Enter your credentials in your browser to access Dione</h2>
                    </div>
                    <div className="mt-12 flex flex-col gap-4">
                    <div className="flex flex-col gap-2 items-center justify-center">
                        <h3 className="text-white/50 text-xs">Could not authenticate?</h3>
                        <button className="bg-white/10 w-28 rounded-full p-1.5 text-sm text-neutral-300 hover:bg-white/20 transition-colors duration-300 cursor-pointer" onClick={() => { setLevel(1);}}>
                            Go back
                        </button>
                    </div>
                    </div>
            ...
            {level === 3 && (
                <div className="w-full h-full flex flex-col  items-center justify-center">
                    ...
                    <h2 className="text-neutral-400 text-balance text-center max-w-xl">
                        We are glad to have you here {JSON.parse(localStorage.getItem("user") || '{}').user_metadata?.name}!
                    </h2>
                    </div>
                    <div className="mt-12 flex flex-col gap-4">
                    <div className="flex flex-col gap-2 items-center justify-center">
                        <Link to="/" className="bg-white/20 transition-all duration-400 w-28 rounded-full p-2 text-sm text-neutral-200 cursor-pointer">
                            <span className="text-center w-full flex items-center justify-center">Finish</span>
                        </Link>
                    </div>
                    </div>
                </div>
