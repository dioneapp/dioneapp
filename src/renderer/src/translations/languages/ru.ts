	...
	settings: {
		applications: {
			...
			binDirectory: {
				...
				description:
					"Выберите, где будут храниться исполняемые файлы приложений для удобного доступа",
			},
			cleanUninstall: {
				label: "Чистая Деинсталляция",
				description:
					"Удалить все связанные зависимости при деинсталляции приложений",
			},
		...
		notifications: {
			...
			installationAlerts: {
				...
				description: "Получать уведомления при завершении установки приложений",
			},
			discordRPC: {
				label: "Discord Rich Presence",
				description: "Показывать текущую активность в статусе Discord",
			},
		},
		privacy: {
		...
		other: {
			...
			disableAutoUpdate: {
				...
				description: "Отключает автоматические обновления. Внимание: ваше приложение может пропустить важные исправления или патчи безопасности. Эта опция не рекомендуется для большинства пользователей.",
			},
			logsDirectory: {
