export type Theme = 'default' | 'midnight' | 'ocean' | 'forest' | 'sunset' | 'royal';
...
export function applyTheme(theme: Theme, intensity?: BackgroundIntensity): void {
	const themeColors = themes[theme];
	const root = document.documentElement;
	const backgroundIntensity = intensity || getStoredBackgroundIntensity();

	root.style.setProperty('--theme-background', themeColors.background[backgroundIntensity]);
	root.style.setProperty('--theme-accent', themeColors.accent);
	root.style.setProperty('--theme-accent-secondary', themeColors.accentSecondary);
