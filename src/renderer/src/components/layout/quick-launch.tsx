...
export default function QuickLaunch({
  ...
  const renderEmptyButton = (index: number) => {
    ...
    const availableToAdd = availableApps.filter((app) => !appsInQuickLaunch.includes(app.id));
    ...
    return (
      ...
      >
        <motion.button
          ...
          transition={{ duration: 0.2 }}
        >
          <Plus className={`h-10 w-10 transition-colors ${isHovered ? "text-[#BCB1E7]" : ""}`} />
        </motion.button>
        <div className="max-w-18 overflow-hidden flex justify-center items-center">
          ...
          >
            ...
            <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 whitespace-nowrap">
              <div className="bg-black/80 backdrop-blur-sm border border-white/20 rounded-md px-2 py-1">
                <p className="text-white text-xs">{dragState.draggedApp.name}</p>
              </div>
            </div>
          </motion.div>
        )}
