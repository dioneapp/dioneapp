...
export default function Messages({
  ...
  logsEndRef,
  quickAI,
}: { messages: any[]; logsEndRef: React.RefObject<HTMLDivElement | null>; quickAI?: boolean }) {
      ...
      {messages.map((message: any, index: number) => (
        ...
        >
          ...
          <div className={`flex flex-col ${quickAI ? "max-w-[70%]" : ""}`}>
            {message?.role !== "user" && message?.message?.tool_calls && message?.message?.tool_calls?.length > 0 && (
              <div className="mb-2">
                {message?.message?.tool_calls[0].function.name === "read_file" && (
                  <span className="text-xs text-gray-500 dark:text-gray-400">Reading files...</span>
                )}
              </div>
            )}
            <div className={`prose prose-invert text-sm leading-relaxed text-pretty ${message?.role === "user" ? "text-neutral-300" : ""}`}>
              <ReactMarkdown
                ...
                components={{
                  code({
                    ...
                    return !inline && match ? (
                      ...
                      <code className="bg-neutral-800 rounded text-sm font-mono" {...props}>
                        {children}
                      </code>
                    )
                  },
