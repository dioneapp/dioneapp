...
export function setupSocket({
    ...
    const applyProgress = (nextPercent: number, label?: string, status: "running"|"success"|"error" = "running") => {
        const smooth = 0.6 * lastPercent + 0.4 * nextPercent;
        lastPercent = smooth;
        setProgress((prev) => ({
            ...prev,
            [appId]: {
                mode: structuredRunActive ? "determinate" : prev[appId]?.mode || "indeterminate",
    ...
    const scheduleApply = (nextPercent: number, label?: string, status?: "running"|"success"|"error") => {
        if (debounceTimer) clearTimeout(debounceTimer);
        debounceTimer = setTimeout(() => applyProgress(nextPercent, label, status as any), 80);
    };

    const computePercent = () => {
        const finishedWeight = steps.reduce((acc, s) => acc + (finished.has(s.id) ? s.weight : 0), 0);
        const currentWeight = steps.find((s) => s.id === currentStepId)?.weight || 0;
        const percent = (finishedWeight + currentWeight * currentStepProgress) * 100;
        return percent;
    };

    // heuristic milestones when no structured run
    const milestones = [
        { re: /all required dependencies are installed/i, weight: 0.15, label: "Dependencies installed" },
        { re: /using virtual environment|creating\/using virtual environment|virtual environment/i, weight: 0.25, label: "Environment ready" },
        { re: /build tools initialized|build tools ready/i, weight: 0.35, label: "Build tools" },
        { re: /working on directory|changed working directory/i, weight: 0.45, label: "Preparing workspace" },
        { re: /executing:/i, weight: 0.55, label: "Executing" },
        { re: /running on|serving at|server running|localhost|127\.0\.0\.1|0\.0\.0\.0|http:\/\/|https:\/\//i, weight: 1.0, label: "Service running" },
    ];
    ...
    socket.on("run_progress", (evt: any) => {
        ...
        switch (evt.type) {
            ...
            case "step_started": {
                ...
                currentStepProgress = 0;
                scheduleApply(computePercent(), steps.find((s) => s.id === currentStepId)?.label);
                break;
            }
            case "step_progress": {
                if (evt.id === currentStepId) {
                    currentStepProgress = Math.max(0, Math.min(1, Number(evt.progress) || 0));
                    scheduleApply(computePercent(), steps.find((s) => s.id === currentStepId)?.label);
                }
                break;
            }
            ...
            case "run_finished": {
                ...
                applyProgress(success ? 100 : lastPercent, success ? "Running" : "Failed", success ? "success" : "error");
                break;
            }
            default:
                break;
        ...
        if (!structuredRunActive) {
            ...
            for (let i = 0; i < milestones.length; i++) {
                if (!seenMilestones.has(i) && milestones[i].re.test(text)) {
                    ...
                    scheduleApply(target, milestones[i].label, /error|failed/i.test(text) ? "error" : "running");
                }
            }
        }
    });
    ...
    socket.on(
        ...
        (message: { type: string; content: string; status: string }) => {
            ...
            if (type === "status") {
                ...
                if (status === "success" && content.toLowerCase().includes("actions executed")) {
