			...
			<div className="w-full h-full flex justify-center items-center">
				<div className="max-w-2xl max-h-96 rounded-xl w-full h-full bg-black/80 border border-white/10 overflow-hidden relative flex flex-col">
					...
					<div className="flex flex-col w-full ml-auto mt-auto justify-end items-end px-6 py-4">
						<button onClick={onClose} className="bg-neutral-200 hover:bg-neutral-300 transition-all duration-300 cursor-pointer text-black text-sm font-semibold py-1 w-24 rounded-full">{t("toast.close")}</button>
						<p onClick={() => openLink(data.author_url || "https://github.com/dioneapp/official-scripts")} className="text-xs text-neutral-400 mt-2 hover:text-neutral-200 transition-all duration-300 cursor-pointer">{t("report.contribute")}</p>
					</div>
				</div>
			</div>
		</div>
