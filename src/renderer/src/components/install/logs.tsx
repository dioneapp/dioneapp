...
export default function LogsComponent({
	...
	return (
		...
		>
			...
			<motion.div className="p-10 rounded-tl-xl rounded-b-xl border-tl-0 border border-white/10 shadow-lg relative overflow-auto w-full bg-[#080808]/80">
				<div className="max-h-96 overflow-auto p-4 pointer-events-auto">
					{logs.map((log, index) => (
						<p
							className={`text-xs whitespace-pre-wrap ${log.startsWith("ERROR") || log.includes("error") ? "text-red-400" : log.startsWith("WARN:") ? "text-yellow-400" : "text-neutral-300"}`}
							key={index}
						>
							{log || "loading"}
						</p>
