...
export function createScriptRouter(io: Server) {
  ...
  // update a script by performing git pull
  router.post("/update/:name/:id", async (req, res) => {
    const { name, id } = req.params;
    const sanitizedName = name.replace(/\s+/g, "-");
    const root = process.cwd();
    const config = readConfig();
    ...
    try {
      // read dione file to get git repository info
      const dioneFile = path.join(workingDir, "dione.json");
      const options = await readDioneConfig(dioneFile);

      if (!options || !options.installation) {
        return res.status(404).json({ error: "No installation configuration found" });
      }

      // find git clone command to extract repository path
      let repoPath: string | null = null;
      for (const step of options.installation) {
        if (step.commands) {
          for (const command of step.commands) {
            if (typeof command === 'string' && command.includes('git clone')) {
              // extract the target directory from git clone command
              const parts = command.split(' ');
              const cloneIndex = parts.findIndex(part => part === 'clone');
              if (cloneIndex !== -1 && parts[cloneIndex + 2]) {
                repoPath = path.join(workingDir, parts[cloneIndex + 2]);
                break;
              }
            }
          }
      ...
      if (!repoPath) {
        return res.status(404).json({ error: "No git repository found in installation steps" });
      }
      ...
      logger.info(`Updating repository at: ${repoPath}`);
      ...
      gitPull.on("close", (code) => {
        if (code === 0) {
          logger.info(`Git pull completed successfully for ${name}`);
          io.to(id).emit("updateProgress", {
            type: "success",
          ...
          logger.error(`Git pull failed for ${name} with code ${code}`);
          io.to(id).emit("updateProgress", {
            type: "error",
            content: `Update failed with code ${code}`
          });
          res.status(500).json({
            success: false,
      ...
      logger.error(
        `Error updating script "${name}": [ (${error.code || "No code"}) ${error.message || error.details || "No details"} ]`,
      );
      res.status(500).json({
        success: false,
        message: "An error occurred while updating the script",
        error: error.message
      });
    }
  });
