...
export async function install(binFolder: string, id: string, io: Server): Promise<{success: boolean}> {
    ...
    const urls: Record<string, Record<string, string>> = {
        ...
        windows: {
            amd64: "https://github.com/git-for-windows/git/releases/download/v2.50.1.windows.1/MinGit-2.50.1-64-bit.zip",
            arm64: "https://github.com/git-for-windows/git/releases/download/v2.50.1.windows.1/MinGit-2.50.1-arm64.zip",
    ...
    const installerFile = fs.createWriteStream(path.join(tempDir, `git-${platform}-${arch}.zip`));
    ...
    const commands: Record<string, { file: string, args: string[] }> = {
        ...
        windows: {
            file: `tar`,
            args: ['-xf', path.join(tempDir, `git-${platform}-${arch}.zip`), '-C', depFolder]
        }
    };
