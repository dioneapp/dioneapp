...
const executeActions = async (
	...
	if (actions.dependencies) {
		...
		if (missingDep.length > 0) {
			...
			io.emit("installUpdate", {
				type: "log",
				content: `ERROR: Aborting... Missing dependencies: ${missingDep.join(", ")}`,
			});
			return;
		}
	...
	try {
		...
		if (actions.start) {
			...
			for (const command of actions.start) {
				...
				if (command.catch) {
					logger.info(
						`Executing script on: http://127.0.0.1://${command.catch}`,
					);
					io.emit("installUpdate", { type: "catch", content: command.catch });
					io.emit("installUpdate", {
						type: "log",
						content: `Watching port ${command.catch}`,
					});
				}
				...
				for (const cmd of command.commands) {
					if (cmd.startsWith("cd")) {
						...
						io.emit("installUpdate", {
							type: "log",
							content: `Changing directory to '${cmd.split(" ")[1]}'`,
						});
						const newDir = cmd.split(" ")[1];
