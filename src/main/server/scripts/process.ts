...
import BuildToolsManager from "../utils/build-tools";
  ...
  try {
    ...
    if (currentPlatform === "win32") {
      return new Promise<boolean>((resolve) => {
        const taskkill = spawn("taskkill", [ // cspell:disable-line
          "/PID",
          pid.toString(),
          "/T",
          "/F",
  ...
  return new Promise((resolve) => {
    exec("netstat -ano -p tcp", async (err, stdout, stderr) => {
      ...
      Promise.all(
        matches.map((pid) => {
          return new Promise<void>((res) => {
            exec(`taskkill /PID ${pid} /T /F`, (killErr, killStderr) => { // cspell:disable-line
              if (killErr) {
