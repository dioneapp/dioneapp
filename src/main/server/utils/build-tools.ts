...
class BuildToolsManager {
	...
	private detectVSInstallations() {
		...
		const defaultBinFolder = settings
			? path.join(settings.defaultBinFolder, "bin") ||
		...
		console.log(
			`Found VS installations: ${installations.map((i) => i.path).join(", ")}`,
		);
	...
	async initialize(): Promise<boolean> {
		...
		try {
			...
			if (installations.length === 0) {
				logger.warn(
					"No Visual Studio installations found - build tools may not work",
				);
				return false;
			}
			...
			try {
				...
				logger.info(
					"Python not found in PATH - some native modules may not compile",
				);
			}
