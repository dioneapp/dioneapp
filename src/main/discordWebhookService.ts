...
const webhookUrl =
	"https://discord.com/api/webhooks/1381353293200101547/jilB77WVJYYJqTfco23-YzTn0JpMZipSRY0xwoMkdb29MDe7PzqNyQHMummuYbUGrUCu";
...
\`\`\``, // Using markdown for code block
	...
	try {
		...
		if (response.ok) {
			...
			console.error(
				`Failed to send error report to Discord. Status: ${response.status}, Body: ${errorBody}`,
			);
		}
	} catch (networkError) {
		console.error(
			"Network error when trying to send error report to Discord:",
			networkError,
		);
	}
}
